  ...
  for (const term of matchedTerms) {
    ...
    for (const idx of pageIdxs) {
      ...
      if (page.lastModified) {
        ...
        if (!isReleaseNotes && (latestMs === null || page.lastModified > latestMs)) {
          latestMs = page.lastModified;
          latestPage = page;
        }
        if (!isBlog && !isReleaseNotes && (latestDocsMs === null || page.lastModified > latestDocsMs)) {
          latestDocsMs = page.lastModified;
        }
      }
    }
  }
  return { latestMs, latestPage, latestDocsMs, docsCount, blogCount };
};
...
const scorePost = ({ post, keywords, docsTokenSet, docsIndex, pages, now = Date.now() }) => {
  ...
  if (keywords.length === 0) {
    ...
    overlapVerdict = 'stale';
  } else if (keywords.length >= ACTIVE_MIN_KEYWORDS && hasFreshDocs) {
