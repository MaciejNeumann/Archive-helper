...
const summarizeMatchedPages = (matchedTerms, docsIndex, pages) => {
  ...
  for (const term of matchedTerms) {
    ...
    for (const idx of pageIdxs) {
      ...
      if (!page) continue;
      if (pageSourceFromUrl(page.url) === 'blog') blogCount++; else docsCount++;
      if (page.lastModified && (latestMs === null || page.lastModified > latestMs)) {
        latestMs = page.lastModified;
        latestPage = page;
      }
    }
  }
  return { latestMs, latestPage, docsCount, blogCount };
};
...
const scorePost = ({ post, keywords, docsTokenSet, docsIndex, pages, now = Date.now() }) => {
  ...
  const { latestMs, latestPage, docsCount, blogCount } = overlap.matchedTerms.length > 0
    ? summarizeMatchedPages(overlap.matchedTerms, docsIndex, pages)
    : { latestMs: null, latestPage: null, docsCount: 0, blogCount: 0 };
  ...
  if (keywords.length > 0) {
    ...
    } else if (overlap.ratio >= 0.6 && latestMs && now - latestMs < RECENT_DOC_WINDOW_MS) {
      score -= 1;
