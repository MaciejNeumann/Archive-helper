...
const LLM_VERDICTS = new Set(['archive', 'keep', 'review']);

const normalizeLlmResult = (r) => ({
  llmVerdict: LLM_VERDICTS.has(r.llmVerdict) ? r.llmVerdict : 'review',
  llmConfidence: typeof r.llmConfidence === 'number'
    ? Math.max(0, Math.min(1, r.llmConfidence))
    : null,
