...
// Subject tokens are repeated this many times to inflate their TF relative to body tokens.
const SUBJECT_BOOST = 4;

// Uses docs-corpus IDF: terms rare in the docs/blog corpus rank higher than generic vocabulary.
const topKeywordsForDocument = (tokens, docsIndex, totalDocsPages, n, vocab) => {
  const tf = new Map();
  ...
  for (const [term, freq] of tf) {
    const docsFreq = Math.max(1, (docsIndex?.get(term) ?? []).length);
    const idf = Math.log(1 + totalDocsPages / docsFreq);
...
const extractKeywordsForPosts = (posts, n = 25, { vocab = null, docsIndex = null, totalDocsPages = 0 } = {}) => {
  const total = totalDocsPages || 1;
  const result = new Map();
  for (const post of posts) {
    const bodyTokens = tokenize(post.body || '');
    const subjectTokens = tokenize(post.subject || '');
    const tokens = [...bodyTokens, ...Array(SUBJECT_BOOST).fill(subjectTokens).flat()];
    const scored = topKeywordsForDocument(tokens, docsIndex, total, n, vocab);
    const maxScore = scored[0]?.score || 1;
    result.set(post, {
      terms: scored.map((s) => s.term),
      scores: Object.fromEntries(scored.map((s) => [s.term, Math.round((s.score / maxScore) * 100)])),
    });
  }
  return result;
};

module.exports = { tokenize, extractKeywordsForPosts, STOP_WORDS, DYNATRACE_GENERIC, SUBJECT_BOOST };
