...
const COMMUNITY_DATE_RE = /^(\d{1,2})\s+([A-Za-z]+)\s+(\d{4})\s+(\d{1,2}):(\d{2})\s+(AM|PM)$/i;
const MONTHS = ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'];

const parseTimeOfPost = (raw) => {
  if (!raw) return null;
  const cleaned = raw.replace(/\s+/g, ' ').trim();
  // ISO-8601 is unambiguous — trust Date.parse for it. Anything else goes through
  // the explicit regex so we don't fall victim to locale-dependent interpretation.
  if (ISO_8601_PREFIX.test(cleaned)) {
    const parsed = Date.parse(cleaned);
    if (!Number.isNaN(parsed)) return new Date(parsed);
  }
  const match = cleaned.match(COMMUNITY_DATE_RE);
